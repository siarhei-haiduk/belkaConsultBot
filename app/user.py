...
@user.message(F.text.lower() == 'вернуться на старт', F.chat.type == 'private')
async def cmd_start(message: Message, state: FSMContext):
    await state.clear()
    await set_user(message.from_user.id, time=None)
    await message.answer(text=TEXT1, reply_markup=await kb.main(message.from_user.id))
...
async def cmd_consult(message: Message, state: FSMContext):
    ...
    await message.answer(text=TEXT4, reply_markup=await kb.contact(message.from_user.id))
...
async def forward_contact(message: Message, state: FSMContext):
    ...
    await message.answer(text=TEXT2, reply_markup=await kb.main(message.from_user.id))
    await state.clear()
